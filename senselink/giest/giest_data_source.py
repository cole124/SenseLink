...
class GIESTSource(DataSource):
    # Primary output property
    _power = 0.0

    def add_controller(self, controller):
        # Add self to passed-in Websocket controller
        if not isinstance(controller, GIESTController):
            raise TypeError(
                f"Incorrect controller type {type(self.controller).__name__} passed to GIEST Data Source")
    ...
    def __init__(self, identifier, details, controller):
        ...
        if details is not None:
            self.outlet_num=str(details.get('outlet_num'))
        #     # Entity ID
    ...
    def parse_bulk_update(self, message):
        ...
        if message is None:
            return
        new_state=message['state']=="on"
        logging.debug(f"Entity update received. Old State: {self.state}, New State: {new_state}")
        ...
        for m in measurement:
            # for ps in measurement[m]['type']:
            if measurement[m]['type']=="realPower":
                new_power=float(measurement[m]['value'])
                logging.debug(f"Power update received. Old VAL: {self.power}, New VAL: {new_power}")
